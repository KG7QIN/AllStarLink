...
#define	BOOST_MAX	40			/* slightly less than 7 bits */
	...
	char alsaradio_write_buf[TX_FRAME_SIZE * 2];    

	int alsaradio_write_dst;
	/* buffers used in alsaradio_read - AST_FRIENDLY_OFFSET space for headers
	 * plus enough room for a full frame
	 */
	char alsaradio_read_buf[RX_FRAME_SIZE * 4 * 6];
	char alsaradio_read_frame_buf[RX_FRAME_SIZE * 2 + AST_FRIENDLY_OFFSET];
#ifdef 	OLD_ASTERISK
...
static void send_sound(struct chan_alsaradio_pvt *o)
{
	...
	for (ofs = 0; ofs < TX_FRAME_SIZE; ofs += l) {
		l = s->samplen - l_sampsent;	/* # of available samples */
		if (l > 0) {
			start = l_sampsent % s->datalen;	/* source offset */
			if (l > TX_FRAME_SIZE - ofs)	/* don't overflow the frame */
				l = TX_FRAME_SIZE - ofs;
			...
			l_sampsent += l;
			static const short silence[TX_FRAME_SIZE] = { 0, };

			l += s->silencelen;
			if (l > 0) {
				if (l > TX_FRAME_SIZE - ofs)
					l = TX_FRAME_SIZE - ofs;
				...
				if (s->repeat == 0) {	/* last block */
					...
					if (ofs < TX_FRAME_SIZE)	/* pad with silence */
						bcopy(silence, myframe + ofs, (TX_FRAME_SIZE - ofs) * 2);
				}
				l_sampsent = 0;
			}
		}
	...
	f.samples = TX_FRAME_SIZE;
	f.datalen = TX_FRAME_SIZE * 2;
	f.data = myframe;
...
static void *sound_thread(void *arg)
{
	...
	for (;;) {
		...
		if (FD_ISSET(o->indev, &rfds)) {
			...
			r = snd_pcm_readi(o->inhandle, buf, RX_FRAME_SIZE);
			if (r == -EPIPE) {
				if (alsaradio_debug)
...
static struct ast_frame *alsa_read(struct chan_alsaradio_pvt *o)
{
	...
	static short __buf[RX_FRAME_SIZE + AST_FRIENDLY_OFFSET / 2];
	short *buf;
	static int readpos = 0;
	static int left = RX_FRAME_SIZE;
	snd_pcm_state_t state;
	int r = 0;
	int off = 0;
	...
	if (readpos >= RX_FRAME_SIZE) {
		/* A real frame */
		readpos = 0;
		left = RX_FRAME_SIZE;
		f.frametype = AST_FRAME_VOICE;
		f.subclass = AST_FORMAT_SLINEAR;
		f.samples = RX_FRAME_SIZE;
		f.datalen = RX_FRAME_SIZE * 2;
		f.data = buf;
		f.offset = AST_FRIENDLY_OFFSET;
		f.src = alsaradio_tech.type;
		f.mallocd = 0;
...
static snd_pcm_t *alsa_card_init(struct chan_alsaradio_pvt *o, char *dev, snd_pcm_stream_t stream)
{
	...
	struct pollfd pfd;
	snd_pcm_uframes_t rx_period_size = RX_FRAME_SIZE;
	snd_pcm_uframes_t tx_period_size = TX_FRAME_SIZE;
	...
	if (stream == SND_PCM_STREAM_PLAYBACK) {
		err = snd_pcm_hw_params_set_period_size_near(handle, hwparams, &tx_period_size, &direction);
		if (err < 0)
			ast_log(LOG_ERROR, "period_size(%ld frames) is bad: %s\n", tx_period_size, snd_strerror(err));
		else {
			ast_log(LOG_NOTICE, "Period size is: %d\n", (int)tx_period_size);
		}
		buffer_size = tx_period_size * 16;
	}
	else {
		err = snd_pcm_hw_params_set_period_size_near(handle, hwparams, &rx_period_size, &direction);
		if (err < 0)
			ast_log(LOG_ERROR, "period_size(%ld frames) is bad: %s\n", rx_period_size, snd_strerror(err));
		else {
			ast_log(LOG_NOTICE, "Period size is: %d\n", (int)rx_period_size);
		}
		buffer_size = rx_period_size * 16;
	}
	...
	if (stream == SND_PCM_STREAM_PLAYBACK)
		start_threshold = tx_period_size / 2;
	else
		start_threshold = 1;
